...
class SmoothAPLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        # Sort the probabilities in descending order for each sample in the batch
        sorted_probs, sorted_indices = torch.sort(probabilities, dim=1, descending=True)
        
        # Gather the labels according to the sorted indices
        sorted_labels = torch.gather(labels, 1, sorted_indices)

        # Calculate cumulative sums of true labels for each sample in the batch
        true_positive_cumsum = torch.cumsum(sorted_labels, dim=1)

        # Calculate the precision at each rank position for each sample
        rank_positions = torch.arange(1, sorted_labels.size(1) + 1, device=logits.device).float()
        precision_at_k = true_positive_cumsum / rank_positions

        # Calculate average precision for each sample in the batch
        average_precision = torch.sum(precision_at_k * sorted_labels, dim=1) / torch.sum(sorted_labels, dim=1)

        # Return the mean of the negative average precision across the batch
